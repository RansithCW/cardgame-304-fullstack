...
function GameScreen() {
  ...
  useEffect(() => {
    getHand().then((res) => {
      ...
      getGameState().then((res) => {
        const trumpsBackend = res.is_trump_set ? res.trump_suit : "Trump not set yet";
  ...
  const handlePlayCard = async (card) => {
    try {
      ...
      const tableWithIds = res.current_table.map(([CardObj, player_id], idx) => ({
        id: idx,
        suit: CardObj.suit,
        rank: CardObj.rank,
      }));
  ...
  return (
    ...
    style={{
      ...
      <div className="flex flex-row">
            ...
            <ShowCards cards={hand} onCardClick={async (card) => {
